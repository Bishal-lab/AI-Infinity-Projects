...
2-Step Verification on the account; the README walks through it. These are the
same two secret names the BFSI digest bot in this repository uses, so a repo
already sending that brief needs no new credentials for this one.
"""
...
def build_message(subject: str, text_body: str, html_body: str) -> EmailMessage:
    ...
    from_name = (os.environ.get("EMAIL_FROM_NAME") or "VP Role Radar").strip()
    message["From"] = formataddr((from_name, sender()))
    message["To"] = ", ".join(recipients())
    message["Subject"] = subject
    message["Date"] = formatdate(localtime=True)
    message["Message-ID"] = make_msgid(domain=sender().split("@")[-1] or "localhost")
    # Mail clients thread by subject; a recurring brief should not collapse into
