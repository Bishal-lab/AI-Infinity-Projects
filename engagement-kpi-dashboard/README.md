...
**By hand.** Open `dashboard.html` in a browser and choose or drop the `.xlsx` exports.

**From a folder.** Drop the exports into `importer/inbox/` and run
`importer/run.bat` (or `./run.sh`). It writes a populated page to `importer/out/`
that is already filled in when you open it — see [`importer/README.md`](importer/README.md).
The importer computes no KPIs: it reads spreadsheets and emits rows, and the page
does every calculation, so the two routes cannot disagree. `test/imported.mjs`
asserts that tile for tile.
...
| `config/sources.json` | the five report signatures — **inlined into the page by `build.sh` and read directly by the importer**, so neither can drift from the other |
| `src/02_sources.js` | the four reading decisions, filters; its `SOURCES` marker is filled from the JSON above |
...
| `node test/imported.mjs` | a page generated by the importer matches the dragged path on all ten tiles, keeps its dates through the JSON round trip, and still makes no requests |
